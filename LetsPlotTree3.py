...
def create_colors(N):
    ...
    colors = []
    for n in xrange(N):
         colors.append(hsv_to_rgb(np.dstack((random( ),1,0.33)))[0][0])
    return colors
...
def blocks(tree,data,plt,next_X):
    ...
    del data["Names"]
    Y_leaf_dist = (max(Y.values( ))-min(Y.values( )))/float(len(tree.get_leaf_names( )))
    X_w = 30 if ncols < 25 else 5
    col_comb = {}
    for col in data.columns:
        allcolor = create_colors(len(set(data[col])))
    ...
    if ncols<25:
        ...
        for col in data.columns:
            plt.text(t_X+X_w/2.0,t_Y,col,fontsize=X_w/(300.0/40.),color='k',ha='center',va='bottom',rotation='vertical')
            t_X +=X_w
    return plt
...
if __name__=='__main__':
    ...
    input_files = ["/home/devil/Documents/Jen/GAS/Final Data/Vir_Emm.csv",
                   "/home/devil/Documents/Jen/GAS/Final Data/test_antibiotics.csv",
                   "/home/devil/Documents/Jen/GAS/Final Data/SuperAntigen.csv"]
    ...
    for fl in input_files:
        ...
        next_X += 30*len(data.columns) + 20  if len(data.columns) < 25 else 5*len(data.columns)+20
    plt.xlim(0,next_X)
